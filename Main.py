...
xyz = 0  # total casualty
a = 0  # total news article processed
...
for filename in os.listdir(corpusDir):
    ...
    for line in data:
        ...
        # splitting text file into news to process one news at a time
        if line == '#####\n':
            x += 1
        if x == 2:
            x = 0
            ...
            death_count, injury_count = NgramMethod.text_processor()

            # insert into database only if there is any injury or death caused by lightning
            # if death_count > 0 or injury_count > 0:
            ...
            #     occurred_on = incident_date.strftime('%Y/%m/%d')
            #
            #     output = occurred_on + '   ' + repr(death_count) + '    ' + repr(injury_count) + domain + '\n'
            #     out.write(output)
            #
            #     sql = "INSERT INTO `processed_news` (`content`, `domain`, `url`, `title`, `news_date`," \
            #           "`original_date`, `casualty_count`, `injury_count`) VALUES (" + repr(content) + ", " +\
            #           repr(domain) + ", " + repr(url) + ", " + repr(title) + ", " + repr(published) + ", " +\
            #           repr(occurred_on) + ", " + repr(death_count) + ", " + repr(injury_count) + ");"
            #     cur.execute(sql)
            cont = open('content.txt', encoding='utf8')
            lns = cont.readlines()
            cont.close()
